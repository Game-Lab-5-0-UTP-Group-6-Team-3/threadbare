...
@export_node_path("Sprite2D") var sprite_path: NodePath = NodePath("ColllsionNumber2/SpriteNumber2")
@export var valor_boton: int = 2
...
func _ready() -> void:
	...
	if interact_area:
		...
		if interact_area.get_parent() == self:
			...
			print("Number2: InteractArea encontrado y conectado correctamente")
		else:
			print("Number2: ERROR - InteractArea no pertenece a este objeto! Parent: ", interact_area.get_parent().name if interact_area.get_parent() else "null")
	else:
...
func _on_interaction_started(_player: Player, _from_right: bool) -> void:
	if _boton_usado:
		print("Number2: ignorando interacción, ya fue utilizado")
		call_deferred("_end_interaction")
		return
	# Esta función se llama cuando el jugador interactúa con el objeto
	print("Number2: _on_interaction_started llamado")
	interactuado2.emit()
	boton_presionado.emit(valor_boton)
	print("¡Number2 fue interactuado!")
	_marcar_como_usado()
...
func _end_interaction() -> void:
	if interact_area:
		print("Number2: Emitiendo interaction_ended")
		interact_area.interaction_ended.emit()
	else:
		print("Number2: ERROR - interact_area es null al intentar finalizar")
