...
func _ready() -> void:
	...
	if interact_area:
		...
		if interact_area.get_parent() == self:
			...
			print("Number5: InteractArea encontrado y conectado correctamente")
		else:
			print("Number5: ERROR - InteractArea no pertenece a este objeto! Parent: ", interact_area.get_parent().name if interact_area.get_parent() else "null")
	else:
...
func _on_interaction_started(_player: Player, _from_right: bool) -> void:
	...
	print("Number5: _on_interaction_started llamado")
	interactuado5.emit()
	print("¡Number5 fue interactuado!")
...
func _end_interaction() -> void:
	if interact_area:
		print("Number5: Emitiendo interaction_ended")
		interact_area.interaction_ended.emit()
	else:
		print("Number5: ERROR - interact_area es null al intentar finalizar")
